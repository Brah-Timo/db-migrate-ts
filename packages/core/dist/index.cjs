...
var SchemaValidator = class {
  ...
  validateColumnDefinition(tableName, columnName, definition, errors, warnings) {
    ...
    if (!(definition.schema instanceof zod.z.ZodType)) {
      errors.push(
    ...
    if (definition.primaryKey && definition.schema instanceof zod.z.ZodOptional) {
      warnings.push(
    ...
    if (definition.schema instanceof zod.z.ZodBoolean && definition.default === void 0 && !definition.nullable) {
      warnings.push(
...
var MigrationBuilderImpl = class {
  ...
  dropTable(tableName, options) {
    this.ops.push({ type: "DROP_TABLE", tableName: String(tableName), ...options !== void 0 ? { options } : {} });
    return this;
  }
  renameTable(from, to) {
    if (String(from) === to) {
  ...
  dropIndex(indexName, options) {
    this.ops.push({ type: "DROP_INDEX", indexName, ...options !== void 0 ? { options } : {} });
    return this;
  }
  // ----------------------------------------------------------
  ...
  raw(sql, bindings) {
    ...
    this.ops.push({ type: "RAW_SQL", sql: trimmed, ...bindings !== void 0 ? { bindings } : {} });
    return this;
  }
  // ----------------------------------------------------------
...
var ZodToSQLConverter = class {
  ...
  resolveType(schema) {
    ...
    if (inner instanceof zod.z.ZodString) return this.resolveString(inner);
    if (inner instanceof zod.z.ZodNumber) return this.resolveNumber(inner);
    if (inner instanceof zod.z.ZodBigInt) return this.resolveBigInt();
    if (inner instanceof zod.z.ZodBoolean) return this.resolveBoolean();
    if (inner instanceof zod.z.ZodDate) return this.resolveDate();
    if (inner instanceof zod.z.ZodEnum) return this.resolveEnum();
    if (inner instanceof zod.z.ZodNativeEnum) return this.resolveNativeEnum(inner);
    if (inner instanceof zod.z.ZodArray) return this.resolveJson();
    if (inner instanceof zod.z.ZodObject) return this.resolveJson();
    if (inner instanceof zod.z.ZodRecord) return this.resolveJson();
    if (inner instanceof zod.z.ZodUnknown) return "TEXT";
    if (inner instanceof zod.z.ZodAny) return "TEXT";
    if (inner instanceof zod.z.ZodLiteral) return this.resolveLiteral(inner);
    if (inner instanceof zod.z.ZodNull) return "TEXT";
    if (inner instanceof zod.z.ZodUndefined) return "TEXT";
    return "TEXT";
  }
  // ----------------------------------------------------------
  //  String Resolution
  ...
  isNullable(definition) {
    ...
    if (schema instanceof zod.z.ZodNullable) return true;
    return false;
  }
  // ----------------------------------------------------------
  //  Constraint Building
  ...
  formatDefault(value, schema) {
    ...
    if (typeof value === "boolean") {
      const inner = this.unwrap(schema);
      if (inner instanceof zod.z.ZodBoolean) {
        switch (this.dialect) {
          case "postgres":
...
var Logger = class {
  ...
  formatMessage(level, message) {
    const parts = [];
    if (this.timestamps) {
      parts.push(chalk2__default.default.gray((/* @__PURE__ */ new Date()).toISOString()));
    }
    if (this.prefix) {
      parts.push(chalk2__default.default.cyan(`[${this.prefix}]`));
    }
    switch (level) {
      case "debug":
        parts.push(chalk2__default.default.gray("[DEBUG]"), chalk2__default.default.gray(message));
        break;
      case "info":
        parts.push(chalk2__default.default.blue("[INFO]"), chalk2__default.default.white(message));
        break;
      case "warn":
        parts.push(chalk2__default.default.yellow("[WARN]"), chalk2__default.default.yellow(message));
        break;
      case "error":
        parts.push(chalk2__default.default.red("[ERROR]"), chalk2__default.default.red(message));
        break;
      case "success":
        parts.push(chalk2__default.default.green("[OK]"), chalk2__default.default.green(message));
        break;
    }
    return parts.join(" ");
  }
  ...
  divider(char = "\u2500", width = 60) {
    if (this.level !== "silent") {
      console.log(chalk2__default.default.gray(char.repeat(width)));
    }
  }
  ...
  table(rows) {
    ...
    console.log(chalk2__default.default.bold.white(header));
    console.log(chalk2__default.default.gray(divider));
...
var MigrationRunner = class {
  ...
  async runMigrationUp(migration, checksum) {
    ...
    process.stdout.write(`  ${chalk2__default.default.gray("\u2192")} ${chalk2__default.default.white(name)}  `);
    ...
    try {
      ...
      if (this.options.dryRun) {
        process.stdout.write(chalk2__default.default.yellow("(dry run)\n"));
        console.log(chalk2__default.default.gray("    SQL:"));
        statements.forEach((sql) => console.log(chalk2__default.default.gray(`      ${sql}`)));
        return;
      }
      ...
      process.stdout.write(chalk2__default.default.green("\u2713") + chalk2__default.default.gray(` (${durationMs}ms)
`));
    } catch (error) {
      process.stdout.write(chalk2__default.default.red("\u2717\n"));
...
`));
  ...
  async saveSnapshot(migrationName, direction, statements) {
    try {
      await promises.mkdir(this.options.snapshotsDir, { recursive: true });
      const filename = `${migrationName}.${direction}.snapshot.sql`;
      const filepath = path.join(this.options.snapshotsDir, filename);
      const content = [
...
var RollbackManager = class {
  constructor(adapter, allMigrations, dialect, migrationsTable) {
    ...
    this.runner = new MigrationRunner(adapter, { dialect, ...migrationsTable !== void 0 && { migrationsTable } });
  }
  adapter;
  allMigrations;
  dialect;
...
async function discoverMigrationFiles(migrationsDir) {
  ...
  try {
    const stats = await promises.stat(absoluteDir);
...
Run "db-migrate-ts generate <name>" to create your first migration.`
  ...
  const entries = await promises.readdir(absoluteDir);
...
exports.DEFAULT_MIGRATIONS_TABLE = DEFAULT_MIGRATIONS_TABLE;
exports.DIALECT_FEATURES = DIALECT_FEATURES;
exports.DatabaseConnectionError = DatabaseConnectionError;
exports.EmptyTableSchemaError = EmptyTableSchemaError;
exports.InvalidRenameError = InvalidRenameError;
exports.Logger = Logger;
exports.MIGRATION_FILE_PATTERN = MIGRATION_FILE_PATTERN;
exports.MigrationBuilderImpl = MigrationBuilderImpl;
...
exports.MigrationsDirNotFoundError = MigrationsDirNotFoundError;
...
exports.UnsupportedDialectOperationError = UnsupportedDialectOperationError;
exports.VERSION = VERSION;
exports.ZodToSQLConverter = ZodToSQLConverter;
...
exports.formatSQLFile = formatSQLFile;
exports.generateTimestamp = generateTimestamp;
exports.highlightSQL = highlightSQL;
exports.loadAllMigrations = loadAllMigrations;
exports.loadMigrationFile = loadMigrationFile;
exports.sanitizeLike = sanitizeLike;
exports.sanitizeMigrationName = sanitizeMigrationName;
